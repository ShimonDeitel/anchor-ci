...
extension Color {
    static let anchorAccent = Color(hex: "#007AFF")          // the single accent
    static let anchorCard = Color(uiColor: .secondarySystemBackground)
    static let anchorCard2 = Color(uiColor: .tertiarySystemBackground)
    static let anchorField = Color(uiColor: .tertiarySystemFill)
    static let anchorHair = Color(uiColor: .separator)
}
...
extension View {
    func anchorCard(cornerRadius: CGFloat = 20) -> some View {
        self.padding(16)
            .background(Color.anchorCard, in: RoundedRectangle(cornerRadius: cornerRadius, style: .continuous))
    }

    func anchorPill() -> some View {
        self.padding(.horizontal, 14).padding(.vertical, 8)
            .background(Color.anchorCard, in: Capsule())
    }
...
struct FilledAccentButtonStyle: ButtonStyle {
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .background(Color.anchorAccent, in: Capsule())
